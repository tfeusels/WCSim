...
G4int WCSimDetectorConstruction::CountPMT(G4int NoPmt)
{
  ...
  G4double theta_min = 13.*M_PI/180; //atan((vessel_radius_curv)/id_spacing)
  ...
  std::cout << "Percentage of covered hemispherical surface above theta_min = " << TotPmt*(1-std::cos(fEta))*100/(1-std::cos(M_PI/2-theta_min)) << "%" << std::endl;
  std::cout << "Test: vNic: " << vNiC.size() << " vAlpha: " << vAlpha.size() << " vCircle: " << vCircle.size() << std::endl;
...
G4double WCSimDetectorConstruction::ComputeEta(G4int NoPmt)
{
  ...
  G4double cospi = std::cos(M_PI/NoPmt);
...
G4int WCSimDetectorConstruction::ComputeNiC (G4double alphaOfCircle, G4double Eta)
{
  ...
  G4int		NiCLocal  = std::floor(M_PI/arccos); //eq. 7
  return NiCLocal;
}
